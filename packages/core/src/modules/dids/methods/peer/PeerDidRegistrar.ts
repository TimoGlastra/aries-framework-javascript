...
export class PeerDidRegistrar implements DidRegistrar {
  ...
  public async create(
    ...
    if (isPeerDidNumAlgo0CreateOptions(options)) {
      ...
      didPeer = DidPeer.fromDidDocument(options.didDocument)
    }
  ...
  public update(): Promise<DidUpdateResult> {
    throw new DidOperationNotSupported('Cannot update did:peer did')
  }

  public deactivate(): Promise<DidDeactivateResult> {
    throw new DidOperationNotSupported('Cannot deactivate did:peer did')
  }
}
...
// TODO: allow to pass in existing key to create document
